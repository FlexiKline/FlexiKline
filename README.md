...
FlexiKline 是一个高度灵活且可定制的 Flutter 金融 K 线图表框架，适用于股票、数字货币等行情展示场景。支持自定义指标、绘制工具、主副图布局和手势操作，兼容多平台（Android、iOS、Web、macOS、Windows、Linux）。
...
- 自定义指标：实现指标配置与绘制对象接口，支持数据指标（MA、MACD、KDJ 等）和业务指标（Trade 等）
- 自定义绘制工具：实现绘制工具接口
- 支持全屏 / 横屏 / 主副区图表宽高设定与动态调整
- 可定制化与持久化样式、配置、参数（包括指标/绘制等所有功能）
- 适配多平台手势操作，且可定制化操作（惯性平移/缩放位置/平移平滑等）
...
| 数据指标 | `DataIndicatorKey` | `DataIndicator` | `DataPaintObject` | MA、MACD、KDJ 等，需要 precompute，占 slot |
| 业务指标 | `BusinessIndicatorKey` | `BusinessIndicator` | `BusinessPaintObject` | Trade 等，由业务数据驱动，不占 slot |
