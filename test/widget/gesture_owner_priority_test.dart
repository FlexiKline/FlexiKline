...
///
/// `flexi_gesture_owner_test.dart` 已按落点逐条钉住判定顺序，本文件补的是端到端那一段：
/// 判定顺序对了，驱动是否也真的落在预期的业务上。真机反馈的困惑正是在这一层观察到的
/// ——十字线已显示，拖动却把整个图表拖走，十字线反而不动。
///
/// 两条放大器让它不是边角情形：`chartZoomSlideBarRect` 是价格轴那一整条全高隐形热区；
/// `isChartZooming` 是粘性状态，且 `zoomingMove` 的领地是整个 `mainRect`，调过一次留白之后
/// 主区内任何单指拖动都会归它。
library;
...
/// 手动指定的滑竿区：贴主图右侧，与 [_modePosition] 不重叠。
///
/// 用手动区而不是让蜡烛指标自动上报，是为了不与自动上报争同一个字段——自动上报每帧都会
/// 按最宽刻度文本重算。
...
/// 建图表、启用 zoom、手动设滑竿区，并把图表推进到「已在 zoom 模式」。
///
/// zoom 模式一旦进入就是粘性的：`onChartZoomEnd` 只在 padding 回到原值时才退出，否则要靠
/// 那个浮动按钮。所以这就是用户调过一次主区留白之后的常驻状态。
Future<FlexiKlineController> _arrangeZooming(WidgetTester tester) async {
  final scene = await pumpChartInListView(tester, spec: _spec, candles: genFlatCandleList());
  addTearDown(() => disposeChart(tester, scene.chart));
  final chart = scene.chart;

  chart.updateGestureConfig(
    (config) => config.copyWith(enableZoom: true, isManualSetZoomRect: true),
  );
  chart.setChartZoomSlideBarRect(_sliderRect);
  await tester.pump();
  expect(chart.chartZoomSlideBarRect, isNot(Rect.zero), reason: '前置条件: 滑竿区必须已生效');

  expect(chart.onChartZoomStart(_sliderRect.center, false), isTrue);
...
void main() {
  ...
  testWidgets('退出模式后 zoom 模式的图表移动仍正常', (tester) async {
    final chart = await _arrangeZooming(tester);
    final panBefore = chart.paintDxOffset;

    // 正向护栏: 降级不能把 zoomingMove 降成永不可达。
    final gesture = await tester.startGesture(
      toChartGlobal(tester, _modePosition),
      pointer: 1,
      kind: PointerDeviceKind.touch,
    ...
    expect(chart.paintDxOffset, isNot(panBefore), reason: '无人认领时仍应归 zoomingMove');
