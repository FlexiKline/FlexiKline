...
/// `PaintGridTicksMixin` 的 resolve：把 `GridTickMode` 分派到取位算法，并接上本对象的
/// 坐标体系。取位算法本身（等分、固定间距、nice 取整）在 `test/utils/grid_tick_test.dart` 里
/// 直接测，本组只测分派、默认 bounds 与值的往返。
///
/// 两处最容易做错的地方：
///
/// 一是 **nice 的值不再由算法一路传下来**，改由画文本时 `dyToValue(check: false)` 现算。
/// 只要 `valueToDy` / `dyToValue` 不是严格互逆，刻度文本就会显示成不等差的数字，而线的位置
/// 看起来完全正常。
///
/// 二是 **nice 在区间不可用时必须退化**，否则加载中的主区一条横线都没有。
library;
...
void main() {
  ...
  group('v2.5.0/网格线/算位置', () {
    testWidgets('横线的 count: 分派到等分, 含两端', (tester) async {
      ...
      expect(dys, [0.0, 60.0, 120.0, 180.0, 240.0, 300.0]);
      expect(dys.first, bounds.top, reason: '含顶端');
      expect(dys.last, bounds.bottom, reason: '含底端');
    });
    ...
    /// 横线的默认 bounds 为 drawableRect: 横线要横穿整个可绘制区。
    testWidgets('默认 bounds: 横线取 drawableRect', (tester) async {
      final object = await arrange(tester);

      final dys = object.horizontalDysOf(const GridTickMode.count(2));
      expect(dys.first, object.drawableRect.top);
      expect(dys[1], closeTo(object.drawableRect.center.dy, 1e-9));
      expect(dys.last, object.drawableRect.bottom);
    });
    ...
    testWidgets('竖线: count 与 size 按宽度切分, 含两端', (tester) async {
      final object = await arrange(tester);
