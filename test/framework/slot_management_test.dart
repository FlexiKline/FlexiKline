...
/// 属性测试：Slot FIFO 复用与 indicatorCount
///
/// **Validates: Requirements 5.2, 5.3, 5.4**
///
/// 验证 IndicatorPaintObjectManager 的 slot 管理核心不变量：
/// 1. indicatorCount 始终等于当前已注册 key 的数量
/// 2. 回收的 slot 按 FIFO 顺序被下一次注册复用
/// 3. 每个已注册的 DataIndicatorKey 拥有唯一的 slot index
...
void main() {
  group(
    ...
    'Property 8: Slot FIFO 复用与 indicatorCount',
    () {
      // ---------------------------------------------------------------
      // 属性 1：indicatorCount 等于当前已注册 key 的数量
      // ---------------------------------------------------------------
      Glados(slotOpsGen, ExploreConfig(numRuns: 100)).test(
        'indicatorCount 始终等于当前已注册 key 的数量',
        (ops) {
          final manager = createManager();
          final registeredKeys = <DataIndicatorKey>{};

          for (final op in ops) {
            switch (op) {
              case TestRegisterOp(:final key):
                manager.registerDataIndicatorKeys([key]);
                registeredKeys.add(key);
              case TestRecycleOp(:final key):
                manager.recycleDataIndicatorSlot(key);
            ...
            expect(
              ...
              reason: '执行 $op 后 indicatorCount 应等于已注册 key 数量 '
      ...
      Glados(any.intInRange(2, 8), ExploreConfig(numRuns: 100)).test(
        ...
        (keyCount) {
          ...
          final keys = List.generate(keyCount, (i) => DataIndicatorKey('fifo_$i'));
          manager.registerDataIndicatorKeys(keys);

          final originalSlots = <DataIndicatorKey, int>{};
          for (final key in keys) {
            originalSlots[key] = manager.getIndicatorDataIndex(key)!;
          }
          ...
          for (int i = 0; i < recycledSlots.length; i++) {
            final newKey = DataIndicatorKey('new_$i');
            manager.registerDataIndicatorKeys([newKey]);
            expect(
              manager.getIndicatorDataIndex(newKey),
      ...
      // ---------------------------------------------------------------
      Glados(slotOpsGen, ExploreConfig(numRuns: 100)).test(
        '每个已注册的 DataIndicatorKey 拥有唯一的 slot index',
        (ops) {
          final manager = createManager();
          ...
          for (int i = 0; i < 10; i++) {
            final key = DataIndicatorKey('key_$i');
